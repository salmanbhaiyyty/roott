...
start_sunshine() {
    ...
    screen -dmS "$SCREEN_SESSION_SUNSHINE" bash -c "export DISPLAY=$DISPLAY_NUM; sunshine 2>&1 | tee /tmp/sunshine.log"
    sleep 5
    ...
    if ! screen -ls 2>/dev/null | grep -q "$SCREEN_SESSION_SUNSHINE"; then
        ...
        echo ""
        if [[ -f /tmp/sunshine.log ]]; then
            echo "Sunshine log (last 10 lines):"
            tail -n 10 /tmp/sunshine.log
        fi
        log_error "Failed to start Sunshine. Check logs above."
    fi
    ...
    if ! pgrep -x sunshine >/dev/null; then
        ...
        DISPLAY="$DISPLAY_NUM" sunshine >/tmp/sunshine.log 2>&1 &
        sleep 3
        if ! pgrep -x sunshine >/dev/null; then
